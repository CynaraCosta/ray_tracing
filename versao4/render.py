...
def render(v_res, h_res, square_side, dist, eye, look_at, up, background_color, objs, lights, ca):
    ...
    center = eye - (w * dist) # centro = E - d*w, onde E é a origem do sistema de coordenada dos raios primários
    ...
    for i in range(v_res):
        for j in range(h_res):
            ...
            sub_side = square_side/5
            subcenter = Q[0,0] # Centro do primeiro pixel (canto superior esquerdo)

            # Centro do subpixel
            grid[0,0] = subcenter + (1/2 * sub_side * (square_side - 1) * v) - (1/2 * sub_side * (sub_side - 1) * u)
            ...
            for sub_i in range(4):
                for sub_j in range(4):
                    ...
                    # Q[i,j] = Q[0,0] + (square_side * (j * u)) - (square_side * (i * v)) -> centro do subpixel
                    grid[sub_i, sub_j] = grid[0,0] + (sub_side * (sub_j * u)) - (sub_side * (sub_i * v))
                    # calcula o Q[sub_i, sub_j] -> ponto dentro de subpixel

                    sub_line = normalize(grid[sub_i, sub_j] - eye)

                    color = filter_two(objs, eye, sub_line, background_color, ca, lights)
            ...
            # Tirar a média da cor
            
            red = red/25
            green = green/25
            blue = blue/25

            placeH = (red, green, blue)
